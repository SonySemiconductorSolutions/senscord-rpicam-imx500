...
V4L2CtrlManager::~V4L2CtrlManager() { close(fd_); }
...
bool V4L2CtrlManager::Open(void) {
  ...
  for (int i = 0; i < 3; i++) {
    const fs::path sys_dir{"/sys/class/video4linux/v4l-subdev" +
...
bool V4L2CtrlManager::SetExtControl(uint32_t id, void *buffer, size_t size,
                                    uint32_t bit) {
  std::lock_guard<std::mutex> lock_controls(control_mutex);
...
bool V4L2CtrlManager::GetExtControl(uint32_t id, void *buffer, size_t size,
                                    uint32_t bit) {
  std::lock_guard<std::mutex> lock_controls(control_mutex);
