...
void bmp_save(std::vector<libcamera::Span<uint8_t>> const &mem,
  ...
  if (fp == NULL) throw std::runtime_error("failed to open file " + filename);
  ...
  try {
    ...
    if (fwrite((uint8_t *)&file_header + 2, sizeof(file_header) - 2, 1, fp) !=
            1 ||
    ...
    for (unsigned int i = 0; i < info.height; i++, ptr += info.stride) {
      if (fwrite(ptr, line, 1, fp) != 1 ||
          (pad != 0 && fwrite(padding, pad, 1, fp) != 1))
        throw std::runtime_error("failed to write BMP file, row " +
                                 std::to_string(i));
    }
