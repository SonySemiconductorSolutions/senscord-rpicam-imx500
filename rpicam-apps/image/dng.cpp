...
static void unpack_10bit(uint8_t const *src, StreamInfo const &info,
                         uint16_t *dest) {
...
static void unpack_12bit(uint8_t const *src, StreamInfo const &info,
                         uint16_t *dest) {
...
static void unpack_16bit(uint8_t const *src, StreamInfo const &info,
                         uint16_t *dest) {
...
static void uncompress(uint8_t const *src, StreamInfo const &info,
                       uint16_t *dest) {
  // In all cases, the *decompressed* image must be a multiple of 8 columns
  // wide.
  ...
  for (unsigned int y = 0; y < info.height; ++y) {
    ...
    for (unsigned int x = 0; x < info.width; x += 8) {
      if (COMPRESS_MODE & 1) {
        uint32_t w0 = 0, w1 = 0;
        for (int b = 0; b < 4; ++b) w0 |= (*sp++) << (b * 8);
        for (int b = 0; b < 4; ++b) w1 |= (*sp++) << (b * 8);
        subBlockFunction(dp, w0);
        subBlockFunction(dp + 1, w1);
        for (int i = 0; i < 8; ++i, ++dp) *dp = postprocess(*dp);
      } else {
        for (int i = 0; i < 8; ++i) *dp++ = postprocess((*sp++) << 8);
      }
    }
  }
}
...
struct Matrix {
  Matrix(float m0, float m1, float m2, float m3, float m4, float m5, float m6,
         float m7, float m8) {
  ...
  Matrix C() const {
    return Matrix(m[4] * m[8] - m[5] * m[7], -(m[3] * m[8] - m[5] * m[6]),
  ...
  Matrix operator*(Matrix const &other) const {
    ...
    for (int i = 0; i < 3; i++)
      for (int j = 0; j < 3; j++)
        result.m[i * 3 + j] = m[i * 3] * other.m[j] +
                              m[i * 3 + 1] * other.m[3 + j] +
                              m[i * 3 + 2] * other.m[6 + j];
    return result;
  }
  Matrix operator*(float const &f) const {
    Matrix result;
    for (int i = 0; i < 9; i++) result.m[i] = m[i] * f;
    return result;
  }
};

void dng_save(std::vector<libcamera::Span<uint8_t>> const &mem,
  ...
  if (bl) {
    // levels is in the order R, Gr, Gb, B. Re-order it for the actual bayer
    // order.
    for (int i = 0; i < 4; i++) {
  ...
  // Use a slightly plausible default CCM in case the metadata doesn't have one
  // (it should!).
  Matrix CCM(1.90255, -0.77478, -0.12777, -0.31338, 1.88197, -0.56858, -0.06001,
             -0.61785, 1.67786);
  auto ccm = metadata.get(controls::ColourCorrectionMatrix);
  if (ccm) {
    CCM = Matrix((*ccm)[0], (*ccm)[1], (*ccm)[2], (*ccm)[3], (*ccm)[4],
                 (*ccm)[5], (*ccm)[6], (*ccm)[7], (*ccm)[8]);
  } else
    LOG_ERROR("WARNING: no CCM metadata found");

  // This maxtrix from
  // http://www.brucelindbloom.com/index.html?Eqn_RGB_XYZ_Matrix.html
  Matrix RGB2XYZ(0.4124564, 0.3575761, 0.1804375, 0.2126729, 0.7151522,
                 0.0721750, 0.0193339, 0.1191920, 0.9503041);
  Matrix CAM_XYZ = (RGB2XYZ * CCM * WB_GAINS).Inv();

  LOG(2, "Black levels " << black_levels[0] << " " << black_levels[1] << " "
                         << black_levels[2] << " " << black_levels[3]
                         << ", exposure time " << exp_time * 1e6 << "us, ISO "
                         << iso);
  ...
  try {
    ...
    if (!tif) throw std::runtime_error("could not open file " + filename);
    ...
    for (unsigned int y = 0; y < (info.height >> 4); y++) {
      for (unsigned int x = 0; x < (info.width >> 4); x++) {
        ...
        uint32_t grey = buf[off] + buf[off + 1] + buf[off + buf_stride_pixels] +
                        buf[off + buf_stride_pixels + 1];
    ...
    // DNG tags for these, which would have made life so much easier, I have no
    // idea.
    ...
    if (lp) {
      double dist =
          (*lp > 0.0) ? (1.0 / *lp) : std::numeric_limits<double>::infinity();
    ...
    // Now got back to the initial IFD and correct the offsets to its
    // sub-thingies
    ...
    // For reasons unknown, the last sub-IFD that we make seems to reappear at
    // the end of the file as IDF1, and some tools (exiftool for example) are
    // prone to complain about it. As far as I can see the code above is doing
    // the correct things, and I can't find any references to this problem
    // anywhere. So frankly I have no idea what is happening - please let us
    // know if you do. Anyway, this bodge appears to make the problem go away...
