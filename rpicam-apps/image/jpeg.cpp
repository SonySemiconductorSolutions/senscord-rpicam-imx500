...
static const unsigned int exif_image_offset =
    20;  // offset of image in JPEG buffer
...
static ExifReadFunction const exif_read_functions[] = {
    ...
    nullptr,  // byte
    nullptr,  // ascii
    exif_read_short,  exif_read_long,  exif_read_rational,
    nullptr,  // sbyte
    nullptr,  // undefined
    exif_read_sshort, exif_read_slong, exif_read_srational};
...
ExifEntry *exif_create_tag(ExifData *exif, ExifIfd ifd, ExifTag tag) {
  ...
  if (!entry) throw std::runtime_error("failed to allocate EXIF entry");
  entry->tag = tag;
...
void exif_set_string(ExifEntry *entry, char const *s) {
  ...
  if (!entry->data) throw std::runtime_error("failed to copy exif string");
...
void exif_read_tag(ExifData *exif, char const *str) {
  ...
  char tag_name[128];
  int bytes_consumed;
  if (sscanf(str, "%4[^.].%127[^=]=%n", ifd_name, tag_name, &bytes_consumed) !=
      2)
  ...
  if (!entry)
    throw std::runtime_error("failed to create entry for EXIF tag " +
                             tag_string + ", please try without this tag");
  if (entry->format == 0) {
    LOG_ERROR("WARNING: format for EXIF tag " << tag_name
                                              << " unknown - ignoring");
    return;
  }
  if (entry->format == EXIF_FORMAT_UNDEFINED) {
    if (exif_exceptions.count(tag)) {
      ...
      LOG_ERROR("WARNING: libexif format for tag "
                << tag_name << " undefined - treating as ASCII");
      entry->format = EXIF_FORMAT_ASCII;
    }
  }
  ...
  for (unsigned i = 0; i < entry->components; i++) {
    ...
    int extra_consumed =
        (exif_read_functions[entry->format])(str + bytes_consumed, dest);
...
static void YUYV_to_JPEG(const uint8_t *input, StreamInfo const &info,
                         ...
                         const unsigned int output_height, const int quality,
                         const unsigned int restart, uint8_t *&jpeg_buffer,
  ...
  while (cinfo.next_scanline < output_height) {
    unsigned int offset =
        ((cinfo.next_scanline * info.height) / output_height) * info.stride;
...
static void YUV420_to_JPEG(const uint8_t *input, StreamInfo const &info,
  ...
  while (cinfo.next_scanline < output_height) {
    ...
    unsigned int offset_uv =
        (((cinfo.next_scanline / 2) * info.height) / output_height) *
        (info.stride / 2);
    for (unsigned int k = 0; k < output_width3; k += 3) {
...
static void create_exif_data(std::vector<libcamera::Span<uint8_t>> const &mem,
                             ...
                             std::string const &cam_model,
                             StillOptions const *options, uint8_t *&exif_buffer,
                             unsigned int &exif_len, uint8_t *&thumb_buffer,
  ...
  try {
    exif = exif_data_new();
    if (!exif) throw std::runtime_error("failed to allocate EXIF data");
    ...
    if (options->thumb_quality) {
      // Add some tags for the thumbnail. We put in dummy values for the
      // thumbnail offset/length to occupy the right amount of space, and fill
      // them in later.

      LOG(2, "Thumbnail dimensions are " << options->thumb_width << " x "
      ...
      ExifEntry *thumb_offset_entry =
          exif_create_tag(exif, EXIF_IFD_1, EXIF_TAG_JPEG_INTERCHANGE_FORMAT);
      exif_set_long(thumb_offset_entry->data, exif_byte_order, 0);
      ExifEntry *thumb_length_entry = exif_create_tag(
          exif, EXIF_IFD_1, EXIF_TAG_JPEG_INTERCHANGE_FORMAT_LENGTH);
      exif_set_long(thumb_length_entry->data, exif_byte_order, 0);

      // We actually have to write out an EXIF buffer to find out how long it
      // is.
      ...
      // Next create the JPEG for the thumbnail, we need to do this now so that
      // we can go back and fill in the correct values for the thumbnail
      // offsets/length.
      ...
      for (; q > 0; q -= 5) {
        ...
        if (thumb_len <
            60000)  // entire EXIF data must be < 65536, so this should be safe
          break;
        free(thumb_buffer);
      ...
      unsigned int offset =
          exif_len - 6;  // do not ask me why "- 6", I have no idea
    ...
    if (exif) exif_data_unref(exif);
    if (exif_buffer) free(exif_buffer);
    if (thumb_buffer) free(thumb_buffer);
    throw;
  }
}

void jpeg_save(std::vector<libcamera::Span<uint8_t>> const &mem,
               StreamInfo const &info, ControlList const &metadata,
               std::string const &filename, std::string const &cam_model,
  ...
  try {
    ...
    create_exif_data(mem, info, metadata, cam_model, options, exif_buffer,
                     exif_len, thumb_buffer, thumb_len);

    // Make the full size JPEG (could probably be more efficient if we had
    // YUV422 or YUV420 planar format).

    jpeg_mem_len_t jpeg_len;
    YUV_to_JPEG((uint8_t *)(mem[0].data()), info, info.width, info.height,
    ...
    if (!fp) throw std::runtime_error("failed to open file " + options->output);
    ...
    if (fwrite(exif_header, sizeof(exif_header), 1, fp) != 1 ||
        ...
        fwrite(jpeg_buffer + exif_image_offset, jpeg_len - exif_image_offset, 1,
               fp) != 1)
      throw std::runtime_error(
          "failed to write file - output probably corrupt");
