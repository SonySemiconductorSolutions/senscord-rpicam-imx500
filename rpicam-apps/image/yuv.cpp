...
static void yuv420_save(std::vector<libcamera::Span<uint8_t>> const &mem,
                        StreamInfo const &info, std::string const &filename,
  ...
  if (options->encoding == "yuv420") {
    ...
    throw std::runtime_error("output format " + options->encoding +
                             " not supported");
}

static void yuyv_save(std::vector<libcamera::Span<uint8_t>> const &mem,
                      StreamInfo const &info, std::string const &filename,
                      StillOptions const *options) {
  if (options->encoding == "yuv420") {
    ...
    try {
      ...
      for (unsigned int j = 0; j < info.height; j++, ptr += info.stride) {
        for (unsigned int i = 0; i < info.width; i++) row[i] = ptr[i << 1];
        if (fwrite(&row[0], info.width, 1, fp) != 1)
          throw std::runtime_error("failed to write file " + filename);
      }
      ptr = (uint8_t *)mem[0].data();
      for (unsigned int j = 0; j < info.height;
           j += 2, ptr += 2 * info.stride) {
...
static void rgb_save(std::vector<libcamera::Span<uint8_t>> const &mem,
                     StreamInfo const &info, std::string const &filename,
                     StillOptions const *options) {
  ...
  try {
    ...
    if (options->encoding == "rgb48") wr_stride *= 2;
    for (unsigned int j = 0; j < info.height; j++, ptr += info.stride) {
...
void yuv_save(std::vector<libcamera::Span<uint8_t>> const &mem,
              StreamInfo const &info, std::string const &filename,
              StillOptions const *options) {
  ...
  else if (info.pixel_format == libcamera::formats::BGR888 ||
           info.pixel_format == libcamera::formats::RGB888 ||
           info.pixel_format == libcamera::formats::BGR161616 ||
           info.pixel_format == libcamera::formats::RGB161616)
