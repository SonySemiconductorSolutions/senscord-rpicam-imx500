...
Preview *make_preview(Options const *options) {
  if (options->nopreview) return make_null_preview(options);
#if QT_PRESENT
  else if (options->qt_preview) {
    Preview *p = make_qt_preview(options);
    if (p) LOG(1, "Made QT preview window");
    return p;
  }
#endif
  else {
    try {
      ...
      if (p) LOG(1, "Made X/EGL preview window");
      return p;
#else
      ...
      try {
        ...
        if (p) LOG(1, "Made DRM preview window");
        return p;
#else
