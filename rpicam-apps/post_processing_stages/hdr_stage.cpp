...
// This post-processing stage can do DRC (Dynamic Range Compression) and
// HDR-like effects. The difference between the two is really only in the
// parameter configuration where DRC would probably take a single image that is
// exposed "about right" whereas this particular HDR method will want several
...
// rpicam-still -o test.jpg --ev -2 --denoise cdn_off --post-process-file
// hdr.json

// Obviously this runs as a post-processing stage on fully-processed 8-bit
// images. Normally I'd rather do HDR in the raw domain where the signals are
// still linear, and there are more bits to play with, but clearly that's not
// possible here. It does mean some of the pixel manipulations, especially when
// it comes to colour, are a bit random. You have been warned. Enjoy!
...
struct LpFilterConfig {
  ...
  Pwl threshold;    // defines the level of pixel differences that will be
                    // smoothed over
};

// A TonemapPoint gives a target value within the full dynamic range where we
// would like the given quantile (actually, inter-quantile mean) in the image's
// histogram to go. Additionally there are limits to how much the current value
...
struct LocalTonemapConfig {
  Pwl pos_strength;     // gain applied to local contrast when brighter than
                        // neighbourhood
  Pwl neg_strength;     // gain applied to local contrast when darker than
                        // neighbourhood
  double colour_scale;  // allows colour saturation to be increased or reduced
                        // slightly
};
...
struct HdrConfig {
  ...
  std::string
      jpeg_filename;  // set this if you want individual jpegs saved as well
};
...
static void add_Y_pixels(int16_t *dest, uint8_t const *src, int width,
                         int stride, int height) {
  for (int y = 0; y < height; y++, src += stride) {
    for (int x = 0; x < width; x++) *(dest++) += src[x];
  }
}
...
void HdrImage::Accumulate(uint8_t const *src, int stride) {
  ...
  for (int y = 0; y < height; y++, src += stride2) {
    for (int x = 0; x < width2; x++) *(dest++) += src[x] - 128;
  }
...
static void forward_pass(std::vector<double> &fwd_pixels,
                         std::vector<double> &fwd_weight_sums,
                         HdrImage const &in, std::vector<double> &weights,
                         std::vector<double> &threshold, int width, int height,
                         int size, double strength)

{
  // (Should probably initialise the top/left elements of
  // fwd_pixels/fwd_weight_sums...)
...
// Low pass IIR filter. We perform a forwards and a reverse pass, finally
// combining the results to get a smoothed but vaguely edge-preserving version
// of the accumulator image. You could imagine implementing alternative (more
...
HdrImage HdrImage::LpFilter(LpFilterConfig const &config) const {
  ...
  for (int d = 0; d <= 30; d++) weights[d] = exp(-d * d / 100.0);
  ...
  // Run the forward pass in other thread, so that the two passes run in
  // parallel.
  std::thread fwd_pass(forward_pass, std::ref(fwd_pixels),
                       std::ref(fwd_weight_sums), std::ref(*this),
                       std::ref(weights), std::ref(threshold), width, height,
                       size, strength);

  // Reverse pass, but otherwise the same as the forward pass. There could be a
  // small saving in omitting it, but it's not huge given that they run in
  // parallel.
  std::vector<double> rev_weight_sums(width * height);
  std::vector<double> rev_pixels(width * height);
  // (Should probably initialise the bottom/right elements of
  // rev_pixels/rev_weight_sums...)
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++, off++)
      out.P(off) = (fwd_pixels[off] * fwd_weight_sums[off] +
                    rev_pixels[off] * rev_weight_sums[off]) /
...
Histogram HdrImage::CalculateHistogram() const {
  ...
  for (int i = 0; i < width * height; i++) bins[P(i)]++;
  return Histogram(&bins[0], dynamic_range);
}

// This creates the tone curve that we apply to the low pass image using the
// list of quantiles and targets in the configuration.
...
// Tonemap the low pass image according to the global tone curve, and add back
// the high pass detail (given by the original pixel minus the low pass
// equivalent).
...
void HdrImage::Tonemap(HdrImage const &lp, HdrConfig const &config) {
  ...
  std::vector<double> pos_strength_lut =
      config.local_tonemap.pos_strength.GenerateLut<double>();
  std::vector<double> neg_strength_lut =
      config.local_tonemap.neg_strength.GenerateLut<double>();
  ...
  for (int y = 0; y < height; y++) {
    ...
    for (int x = 0; x < width; x++, off_Y++) {
      ...
      double strength =
          (Y_hp > 0 ? pos_strength_lut : neg_strength_lut)[Y_lp_orig];
      int Y_final = std::clamp(Y_lp_mapped + (int)(strength * Y_hp), 0, maxval);
      P(off_Y)    = Y_final;
      if (!(x & 1) && !(y & 1)) {
        double f = (Y_final + 1) / (double)(Y_lp_orig + 1);
        // The values here are non-linear to colours can come out slightly
        // saturated. The colour_scale allows us to tweak that a little if we
        // want.
...
void HdrImage::Extract(uint8_t *dest, int stride) const {
  ...
  const int16_t *U_ptr = Y_ptr + width * height,
                *V_ptr = U_ptr + width * height / 4;
  uint8_t *dest_y      = dest;
  uint8_t *dest_u      = dest_y + stride * height,
          *dest_v      = dest_u + stride * height / 4;

  for (int y = 0; y < height; y++, dest_y += stride) {
    for (int x = 0; x < width; x++) dest_y[x] = *(Y_ptr++) / ratio;
  }
...
void HdrImage::Scale(double factor) {
  for (unsigned int i = 0; i < pixels.size(); i++) pixels[i] *= factor;
...
class HdrStage : public PostProcessingStage {
 public:
  ...
  void AdjustConfig(std::string const &use_case,
...
char const *HdrStage::Name() const { return NAME; }
...
void HdrStage::Read(boost::property_tree::ptree const &params) {
  ...
  config_.global_tonemap.strength =
      params.get<double>("global_tonemap_strength");
  ...
  // A strength of 1 should give the value in the function; a strength of 0
  // should give the value 1.
...
void HdrStage::Configure() {
  ...
  frame_num_ = 0;
  acc_ =
      HdrImage(info_.width, info_.height, info_.width * info_.height * 3 / 2);
  acc_.Clear();
...
bool HdrStage::Process(CompletedRequestPtr &completed_request) {
  if (!stream_) return false;  // in viewfinder mode, do nothing
  ...
  if (frame_num_ >= config_.num_frames) return false;
  ...
  // Optionally save individual JPEGs of each of the constituent images.
  // Obviously this will rather slow down the accumulation process.
  if (!config_.jpeg_filename.empty()) {
    char filename[128];
    snprintf(filename, sizeof(filename), config_.jpeg_filename.c_str(),
             frame_num_);
    filename[sizeof(filename) - 1] = 0;
    StillOptions const *options =
        dynamic_cast<StillOptions *>(app_->GetOptions());
    if (options)
      jpeg_save(buffers, info_, completed_request->metadata, filename,
                app_->CameraModel(), options);
    else
      LOG(1, "No still options - unable to save JPEG");
  }

  // Now we'll drop this frame unless it's the last one that we need, at which
  // point we do our HDR processing and send that through.
  frame_num_++;
  if (frame_num_ < config_.num_frames) return true;
...
static PostProcessingStage *Create(RPiCamApp *app) { return new HdrStage(app); }
