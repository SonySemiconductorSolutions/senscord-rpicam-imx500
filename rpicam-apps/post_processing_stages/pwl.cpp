...
Pwl::Interval Pwl::Range() const {
  ...
  for (auto &p : points_) lo = std::min(lo, p.y), hi = std::max(hi, p.y);
  return Interval(lo, hi);
}

bool Pwl::Empty() const { return points_.empty(); }

double Pwl::Eval(double x, int *span_ptr, bool update_span) const {
  int span = findSpan(x, span_ptr && *span_ptr != -1 ? *span_ptr
                                                     : points_.size() / 2 - 1);
  if (span_ptr && update_span) *span_ptr = span;
  return points_[span].y + (x - points_[span].x) *
                               (points_[span + 1].y - points_[span].y) /
...
int Pwl::findSpan(double x, int span) const {
  ...
  while (span < last_span && x >= points_[span + 1].x) span++;
  while (span && x < points_[span].x) span--;
  return span;
}
...
Pwl Pwl::Compose(Pwl const &other, const double eps) const {
  ...
  while (this_span != (int)points_.size() - 1) {
    ...
    if (abs(dy) > eps && other_span + 1 < (int)other.points_.size() &&
      ...
      this_x =
          points_[this_span].x +
          (other.points_[other_span + 1].x - points_[this_span].y) * dx / dy;
...
void Pwl::Map(std::function<void(double x, double y)> f) const {
  for (auto &pt : points_) f(pt.x, pt.y);
}
