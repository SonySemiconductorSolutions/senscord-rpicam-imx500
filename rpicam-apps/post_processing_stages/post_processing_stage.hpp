...
class PostProcessingStage {
 public:
  ...
  virtual void AdjustConfig(std::string const &use_case,
  ...
  // Below here are some helpers provided for the convenience of derived
  // classes.

  // Convert YUV420 image to RGB. We crop from the centre of the image if the
  // src image is larger than the destination.
  static std::vector<uint8_t> Yuv420ToRgb(const uint8_t *src,
                                          StreamInfo &src_info,
                                          StreamInfo &dst_info);
  static void Yuv420ToRgb(uint8_t *dst, const uint8_t *src,
                          StreamInfo &src_info, StreamInfo &dst_info);

 protected:
  // Helper to calculate the execution time of any callable object and return it
  // in as a std::chrono::duration. For functions returning a value, the
  // simplest thing would be to wrap the call in a lambda and capture the return
  // value.
  template <class R = std::micro, class T = std::chrono::steady_clock, class F,
            class... Args>
  ...
  template <typename T>
  static std::vector<T> GetJsonArray(const boost::property_tree::ptree &pt,
                                     const std::string &key,
                                     const std::vector<T> &default_value = {}) {
    std::vector<T> vec;

    if (pt.find(key) != pt.not_found()) {
      for (auto &v : pt.get_child(key)) vec.push_back(v.second.get_value<T>());
    }
