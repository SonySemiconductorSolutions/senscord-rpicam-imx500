...
void PostProcessingStage::AdjustConfig(std::string const &,
                                       StreamConfiguration *) {}
...
void PostProcessingStage::SetInferenceRoiAbs(
    const libcamera::Rectangle &roi_) const {}

std::vector<uint8_t> PostProcessingStage::Yuv420ToRgb(const uint8_t *src,
                                                      StreamInfo &src_info,
                                                      StreamInfo &dst_info) {
...
void PostProcessingStage::Yuv420ToRgb(uint8_t *dst, const uint8_t *src,
                                      StreamInfo &src_info,
                                      StreamInfo &dst_info) {
  assert(src_info.width >= dst_info.width &&
         src_info.height >= dst_info.height);
  int off_x      = ((src_info.width - dst_info.width) / 2) & ~1,
      off_y      = ((src_info.height - dst_info.height) / 2) & ~1;
  int src_Y_size = src_info.height * src_info.stride,
      src_U_size = (src_info.height / 2) * (src_info.stride / 2);

  // We're going to process 4x2 pixel blocks, as far as alignment allows.
  unsigned int dst_h_aligned = dst_info.height & ~1,
               dst_w_aligned = dst_info.width & ~3;

  unsigned int y = 0;
  for (; y < dst_h_aligned; y += 2) {
    const uint8_t *src_Y0 = src + (y + off_y) * src_info.stride + off_x;
    const uint8_t *src_U  = src + src_Y_size +
                           ((y + off_y) / 2) * (src_info.stride / 2) +
                           off_x / 2;
