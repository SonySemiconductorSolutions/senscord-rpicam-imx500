...
constexpr unsigned int NUM_HEATMAPS =
    NUM_KEYPOINTS * MAP_SIZE.width * MAP_SIZE.height;
constexpr unsigned int NUM_SHORT_OFFSETS =
    2 * NUM_KEYPOINTS * MAP_SIZE.width * MAP_SIZE.height;
...
struct AdjacencyList {
  explicit AdjacencyList(const int num_nodes)
      : child_ids(num_nodes), edge_ids(num_nodes) {}
...
struct KeypointWithScore {
  KeypointWithScore(const Point &_pt, const int _id, const float _score)
      : point(_pt), id(_id), score(_score) {}

  [[maybe_unused]] friend std::ostream &operator<<(
      std::ostream &ost, const KeypointWithScore &keypoint) {
...
float log_odds(const float x) { return -std::log(1.0f / (x + 1E-6) - 1.0f); }
...
std::vector<float> format_tensor(const float *data, unsigned int size,
                                 unsigned int div) {
    ...
    for (unsigned int j = 0; j < MAP_SIZE.width; j++) {
      for (unsigned int k = 0; k < MAP_SIZE.height; k++) {
        tensor[(size * MAP_SIZE.width * k) + (size * j) + i] =
            data[(MAP_SIZE.width * MAP_SIZE.height * i) +
                 (j * MAP_SIZE.height) + k] /
            div;
      }
    }
...
bool pass_keypoint_nms(std::vector<PoseKeypoints> poses, const size_t num_poses,
                       const KeypointWithScore &keypoint,
                       const float squared_nms_radius) {
  for (unsigned int i = 0; i < num_poses; ++i) {
    if (squared_distance(keypoint.point, poses[i][keypoint.id]) <=
        squared_nms_radius)
      return false;
  }
...
void decreasing_arg_sort(std::vector<int> &indices, const float *scores,
...
// Finds the indices of the scores if we sort them in decreasing order.
void decreasing_arg_sort(std::vector<int> &indices,
...
void build_linear_interpolation(const float x, const int n, int *x_floor,
...
void build_bilinear_interpolation(const Point point,
                                  const unsigned int num_channels,
                                  int *top_left, int *top_right,
                                  int *bottom_left, int *bottom_right,
                                  float *y_lerp, float *x_lerp) {
  int y_floor;
  int y_ceil;
  build_linear_interpolation(point.y, MAP_SIZE.height, &y_floor, &y_ceil,
                             y_lerp);
  int x_floor;
  int x_ceil;
  build_linear_interpolation(point.x, MAP_SIZE.width, &x_floor, &x_ceil,
                             x_lerp);
...
std::vector<float> sample_tensor_at_multiple_channels(
    const std::vector<float> &tensor, const Point &point,
    const std::vector<int> &result_channels, unsigned int num_channels) {
  int top_left, top_right, bottom_left, bottom_right;
  float y_lerp, x_lerp;

  build_bilinear_interpolation(point, num_channels, &top_left, &top_right,
                               &bottom_left, &bottom_right, &y_lerp, &x_lerp);

  std::vector<float> result;
  for (auto &c : result_channels) {
    result.push_back((1 - y_lerp) * ((1 - x_lerp) * tensor[top_left + c] +
...
float sample_tensor_at_single_channel(const std::vector<float> &tensor,
                                      const Point &point,
                                      unsigned int num_channels, const int c) {
  std::vector<float> result = sample_tensor_at_multiple_channels(
      tensor, point, std::vector<int>{c}, num_channels);
  return result[0];
}

KeypointQueue build_keypoint_queue(const std::vector<float> &scores,
      ...
      for (unsigned int j = 0; j < NUM_KEYPOINTS; ++j) {
        ...
        if (score >= score_threshold) {
          ...
          const unsigned int y_start =
              std::max((int)y - (int)local_maximum_radius, 0);
          const unsigned int y_end =
              std::min(y + local_maximum_radius + 1, MAP_SIZE.height);
          for (unsigned int y_current = y_start; y_current < y_end;
               ++y_current) {
            const unsigned int x_start =
                std::max((int)x - (int)local_maximum_radius, 0);
            const unsigned int x_end =
                std::min(x + local_maximum_radius + 1, MAP_SIZE.width);
            for (unsigned int x_current = x_start; x_current < x_end;
                 ++x_current) {
              if (scores[y_current * MAP_SIZE.width * NUM_KEYPOINTS +
          ...
          if (local_maximum) {
            ...
            const float y_refined =
                std::clamp(y + dy, 0.0f, MAP_SIZE.height - 1.0f);
            const float x_refined =
                std::clamp(x + dx, 0.0f, MAP_SIZE.width - 1.0f);
...
Point find_displaced_position(const std::vector<float> &short_offsets,
                              const std::vector<float> &mid_offsets,
                              const Point &source, const int edge_id,
                              const int target_id,
  ...
  std::vector<float> offsets = sample_tensor_at_multiple_channels(
      mid_offsets, source, channels, 2 * 2 * NUM_EDGES);
  ...
  for (int i = 0; i < offset_refinement_steps; ++i) {
    offsets = sample_tensor_at_multiple_channels(short_offsets, Point{y, x},
                                                 channels, 2 * NUM_KEYPOINTS);
    y       = std::clamp(y + offsets[0], 0.0f, MAP_SIZE.height - 1.0f);
    x       = std::clamp(x + offsets[1], 0.0f, MAP_SIZE.width - 1.0f);
  }
...
void backtrack_decode_pose(
    const std::vector<float> &scores, const std::vector<float> &short_offsets,
  ...
  const float root_score = sample_tensor_at_single_channel(
      scores, root.point, root.id, NUM_KEYPOINTS);
  ...
  while (!decode_queue.empty()) {
    ...
    for (int j = 0; j < num_children; ++j) {
      ...
      if (edge_id > NUM_EDGES) edge_id += NUM_EDGES;

      const Point child_point = find_displaced_position(
          short_offsets, mid_offsets, current_keypoint.point, edge_id, child_id,
          offset_refinement_steps);
      const float child_score = sample_tensor_at_single_channel(
          scores, child_point, NUM_KEYPOINTS, child_id);
      decode_queue.emplace(child_point, child_id, child_score);
    }
  }
}

void find_overlapping_keypoints(std::vector<bool> &mask,
                                const PoseKeypoints &pose1,
                                const PoseKeypoints &pose2,
                                const float squared_radius) {
  for (unsigned int k = 0; k < mask.size(); ++k) {
    if (squared_distance(pose1[k], pose2[k]) <= squared_radius) mask[k] = true;
  }
}

void perform_soft_keypoint_NMS(
  ...
  for (int i = 0; i < num_instances; ++i) {
    ...
    for (int j = 0; j < i; ++j) {
      ...
      find_overlapping_keypoints(
          keypoint_occluded, all_keypoint_coords[current_index],
...
class PoseNet : public IMX500PostProcessingStage {
 ...
 private:
  std::vector<PoseResults> decodeAllPoses(
      const std::vector<float> &scores, const std::vector<float> &short_offsets,
      const std::vector<float> &mid_offsets);
  void translateCoordinates(std::vector<PoseResults> &results,
...
char const *PoseNet::Name() const { return NAME; }
...
void PoseNet::Read(boost::property_tree::ptree const &params) {
  ...
  offset_refinement_steps_ =
      params.get<unsigned int>("offset_refinement_steps", 5);
...
bool PoseNet::Process(CompletedRequestPtr &completed_request) {
  ...
  if (!raw_stream_ || !scaler_crop) {
    LOG_ERROR(
        "Must have RAW stream and scaler crop available to get sensor "
        "dimensions!");
    return false;
  }
  ...
  std::vector<float> scores =
      format_tensor((float *)output->data(),
                    NUM_HEATMAPS / (MAP_SIZE.width * MAP_SIZE.height), 1);
  std::vector<float> short_offsets = format_tensor(
      (float *)output->data() + NUM_HEATMAPS,
      NUM_SHORT_OFFSETS / (MAP_SIZE.width * MAP_SIZE.height), STRIDE);
  std::vector<float> mid_offsets = format_tensor(
      (float *)output->data() + NUM_HEATMAPS + NUM_SHORT_OFFSETS,
      NUM_MID_OFFSETS / (MAP_SIZE.width * MAP_SIZE.height), STRIDE);

  std::vector<PoseResults> results =
      decodeAllPoses(scores, short_offsets, mid_offsets);
  ...
  if (locations.size() && locations.size() == confidences.size()) {
    completed_request->post_process_metadata.Set("pose_estimation.locations",
                                                 locations);
    completed_request->post_process_metadata.Set("pose_estimation.confidences",
                                                 confidences);
  }
...
std::vector<PoseResults> PoseNet::decodeAllPoses(
    const std::vector<float> &scores, const std::vector<float> &short_offsets,
    const std::vector<float> &mid_offsets) {
  const float min_score_logit = log_odds(threshold_);

  KeypointQueue queue =
      build_keypoint_queue(scores, short_offsets, min_score_logit);
  ...
  while (pose_counter < max_detections_ && !queue.empty()) {
    ...
    if (!pass_keypoint_nms(scratch_poses, pose_counter, root,
    ...
    backtrack_decode_pose(scores, short_offsets, mid_offsets, root,
                          adjacency_list, next_pose, next_scores,
                          offset_refinement_steps_);
  ...
  perform_soft_keypoint_NMS(all_instance_scores, decreasing_indices,
                            scratch_poses, scratch_keypoint_scores,
                            nms_radius_ * nms_radius_);
  ...
  for (int index : decreasing_indices) {
    if (all_instance_scores[index] < threshold_) break;
    ...
    std::copy(scratch_keypoint_scores[index].begin(),
              scratch_keypoint_scores[index].end(),
...
void PoseNet::translateCoordinates(std::vector<PoseResults> &results,
                                   const Rectangle &scaler_crop) const {
  for (auto &r : results) {
    for (auto &keypoint : r.pose_keypoints) {
      std::vector<float> coords{keypoint.x / (INPUT_TENSOR_SIZE.width - 1),
                                keypoint.y / (INPUT_TENSOR_SIZE.height - 1), 0,
                                0};
...
void PoseNet::filterOutputObjects(const std::vector<PoseResults> &results) {
  ...
  for (auto &lt_r : lt_results_) lt_r.matched = false;
  ...
  for (auto const &r : results) {
      ...
      if (found) {
        ...
        for (unsigned int i = 0; i < NUM_KEYPOINTS; i++) {
          lt_r.results.pose_keypoint_scores[i] =
              factor_ * r.pose_keypoint_scores[i] +
              (1 - factor_) * lt_r.results.pose_keypoint_scores[i];
          lt_r.results.pose_keypoints[i].x =
              factor_ * r.pose_keypoints[i].x +
              (1 - factor_) * lt_r.results.pose_keypoints[i].x;
          lt_r.results.pose_keypoints[i].y =
              factor_ * r.pose_keypoints[i].y +
              (1 - factor_) * lt_r.results.pose_keypoints[i].y;
        }

        // Reset the visibility counter for when the result next disappears.
        lt_r.visible = visible_frames_;
        // Decrement the hidden counter until the result becomes visible in the
        // list.
    ...
    // Add the result to the long term list if not found.  This result will
  ...
  for (auto &lt_r : lt_results_) {
    if (!lt_r.matched) {
      // If a non matched result in the long term list is still hidden, set
...
PostProcessingStage *Create(RPiCamApp *app) { return new PoseNet(app); }
