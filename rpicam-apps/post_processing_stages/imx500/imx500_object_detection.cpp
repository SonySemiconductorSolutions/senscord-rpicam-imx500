...
class ObjectDetection : public IMX500PostProcessingStage {
 ...
 private:
  int processOutputTensor(std::vector<Detection> &objects,
...
char const *ObjectDetection::Name() const { return NAME; }
...
bool ObjectDetection::Process(CompletedRequestPtr &completed_request) {
  ...
  auto info =
      completed_request->metadata.get(controls::rpi::CnnOutputTensorInfo);
  ...
  // access to the lt_objects_ state object.
  std::scoped_lock<std::mutex> l(lt_lock_);

  if (output && info) {
    std::vector<float> output_tensor(output->data(),
                                     output->data() + output->size());
    CnnOutputTensorInfo output_tensor_info =
        *reinterpret_cast<const CnnOutputTensorInfo *>(info->data());

    processOutputTensor(objects, output_tensor, output_tensor_info,
                        *scaler_crop);
    ...
    if (temporal_filtering_) {
      ...
      // If no temporal filtering, make sure we fill lt_objects_ with the
      // current result as it may be used if there is no output tensor on
      // subsequent frames.
      lt_objects_.clear();
      for (auto const &obj : objects) lt_objects_.push_back({obj, 0, 0, 0});
    }
  } else {
...
static int createObjectDetectionData(ObjectDetectionOutput &output,
  ...
  if (num_detections > total_detections) {
    LOG(1, "Unexpected value for num_detections: "
               << num_detections << ", setting it to " << total_detections);
...
int ObjectDetection::processOutputTensor(
    std::vector<Detection> &objects, const std::vector<float> &output_tensor,
    const CnnOutputTensorInfo &output_tensor_info,
    const Rectangle &scaler_crop) const {
  if (output_tensor_info.num_tensors != 4) {
    LOG_ERROR("Invalid number of tensors " << output_tensor_info.num_tensors
                                           << ", expected 4");
    return -1;
  }

  const unsigned int total_detections =
      output_tensor_info.info[0].tensor_data_num / 4;
  ...
  if (output_tensor.size() != 6 * total_detections + 1) {
    LOG_ERROR("Invalid tensor size " << output_tensor.size() << ", expected "
                                     << 6 * total_detections + 1);
    return -1;
  }
  ...
  for (unsigned int i = 0; i < std::min(output.num_detections, max_detections_);
       i++) {
    ...
    // Extract bounding box co-ordinates in the inference image co-ordinates and
    // convert to the final ISP output co-ordinates.
    ...
    const Rectangle obj_scaled =
        ConvertInferenceCoordinates(coords, scaler_crop);

    objects.emplace_back(class_index, classes_[class_index], output.scores[i],
                         obj_scaled.x, obj_scaled.y, obj_scaled.width,
                         obj_scaled.height);
  }
...
void ObjectDetection::filterOutputObjects(std::vector<Detection> &objects) {
  ...
  for (auto const &object : objects) {
    ...
    // remain hidden for hidden_frames_ consecutive frames unless the long term
    // list started out empty.
    if (!matched) {
