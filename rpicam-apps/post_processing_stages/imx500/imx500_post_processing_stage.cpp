  ...
  for (unsigned int i = 0; i < 16; i++) {
    ...
    if (fs::exists(module_dir) && fs::is_symlink(module_dir)) {
      ...
      if (ln.string().find("imx500") != std::string::npos) {
        ...
        const fs::path imx500_progress{
            "/sys/kernel/debug/imx500-fw:" + imx500_device_id + "/fw_progress"};
        const fs::path spi_progress{"/sys/kernel/debug/rp2040-spi:" +
                                    spi_device_id + "/transfer_progress"};
...
void IMX500PostProcessingStage::Read(
  ...
  if (params.find("save_input_tensor") != params.not_found()) {
    ...
    norm_val_   = PostProcessingStage::GetJsonArray<int32_t>(pt, "norm_val",
                                                             {0, 0, 0, 0});
    norm_shift_ = PostProcessingStage::GetJsonArray<uint8_t>(pt, "norm_shift",
                                                             {0, 0, 0, 0});
    div_val_ =
        PostProcessingStage::GetJsonArray<int16_t>(pt, "div_val", {1, 1, 1, 1});
  ...
  if (ret) throw std::runtime_error("failed to set network fw ioctl");
  ...
  LOG(1,
      "\n----------------------------------------------------------------------"
      "--------------------------------------------\n"
      "NOTE: Loading network firmware onto the IMX500 can take several "
      "minutes, please do not close down the application."
      "\n----------------------------------------------------------------------"
      "--------------------------------------------\n");
}
...
bool IMX500PostProcessingStage::Process(
  ...
  if (input && input_tensor_file_.is_open()) {
    // There is a chance that this may be called through multiple threads, so
    // serialize the file access.
    ...
    for (unsigned int i = 0; i < input->size(); i++) {
      ...
      sample                     = (sample << norm_shift_[channel]) -
               conv_reg_signed(norm_val_[channel]);
      sample = ((sample << div_shift_) / div_val_[channel]) & 0xFF;
      input_tensor_file_.put(static_cast<uint8_t>(sample));
    }

    if (--save_frames_ == 0) input_tensor_file_.close();
  }

  return false;
}

void IMX500PostProcessingStage::RotateInputTensor(const uint32_t angle) {
  v4l2_control ctrl{ROTATE_CTRL_ID, static_cast<int32_t>(angle)};
  int ret = ioctl(device_fd_, VIDIOC_S_CTRL, &ctrl);
  if (ret) LOG_ERROR("IMX500: Unable to rotate input tensor");
}
...
void IMX500PostProcessingStage::FlipInputTensor(const uint32_t flip) {
  ...
  int ret = ioctl(device_fd_, VIDIOC_S_CTRL, &ctrl);
  if (ret) LOG_ERROR("IMX500: Unable to flip input tensor");
}

Rectangle IMX500PostProcessingStage::ConvertInferenceCoordinates(
    const std::vector<float> &coords, const Rectangle &scaler_crop) const {
  ...
  const Rectangle sensor_crop =
      scaler_crop.scaledBy(sensor_output_size, full_sensor_resolution_.size());
  ...
  const Rectangle obj_translated =
      obj_bound.translatedBy(-sensor_crop.topLeft());
  // -> and finally scaled to the ISP output.
  const Rectangle obj_scaled =
      obj_translated.scaledBy(isp_output_size, sensor_crop.size());

  LOG(2, obj << " -> (sensor) " << obj_sensor << " -> (bound) " << obj_bound
             << " -> (translate) " << obj_translated << " -> (scaled) "
             << obj_scaled);

  return obj_scaled;
}

void IMX500PostProcessingStage::SetInferenceRoiAbs(
    const Rectangle &roi_) const {
  Rectangle roi               = roi_.boundedTo(full_sensor_resolution_);
  const uint32_t roi_array[4] = {(uint32_t)roi.x, (uint32_t)roi.y,
                                 (uint32_t)roi.width, (uint32_t)roi.height};
  ...
  if (ret) LOG_ERROR("IMX500: Unable to set absolute ROI");
}

void IMX500PostProcessingStage::SetInferenceRoiAuto(
    const unsigned int width, const unsigned int height) const {
  Size s =
      full_sensor_resolution_.size().boundedToAspectRatio(Size(width, height));
  Rectangle r = s.centeredTo(full_sensor_resolution_.center())
                    .enclosedIn(full_sensor_resolution_);
  SetInferenceRoiAbs(r);
}

void IMX500PostProcessingStage::ShowFwProgressBar() {
  if (fw_progress_.is_open() && fw_progress_chunk_.is_open() &&
      !progress_thread_running_) {
    progress_thread_running_ = true;
    progress_thread_ =
        std::thread(&IMX500PostProcessingStage::doProgressBar, this);
  }
}
...
std::vector<unsigned int> split(std::stringstream &stream) {
  ...
  while (stream >> n) result.push_back(n);
...
void IMX500PostProcessingStage::doProgressBar() {
  while (progress_thread_running_) {
    ...
    if (progress.size() == 3 && progress[0] == 2) {
      ...
      std::cerr << "Network Firmware Upload: " << current * 100 / progress[2]
                << "% (" << current / 1024 << "/" << progress[2] / 1024
                << " KB)\r";
      std::cerr.flush();
