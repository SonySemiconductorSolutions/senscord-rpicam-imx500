...
char const *NegateStage::Name() const { return NAME; }

void NegateStage::Configure() { stream_ = app_->GetMainStream(); }
...
bool NegateStage::Process(CompletedRequestPtr &completed_request) {
  ...
  for (unsigned int i = 0; i < buffer.size(); i += 4) *(ptr++) ^= 0xffffffff;
