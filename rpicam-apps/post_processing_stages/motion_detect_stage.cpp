...
// compares pixels in the current low res image against the value in the
// corresponding location in the previous one. If it exceeds a threshold it gets
// counted as "different". If enough pixels are different, that indicates
// "motion". A low res image of something like 128x96 is probably more than
// enough, and you can always subsample with hskip and vksip.
...
// The stage adds "motion_detect.result" to the metadata. When this claims
// motion, the application can take that as true immediately. To be sure there's
// no motion, an application should probably wait for "a few frames" of "no
// motion".
...
class MotionDetectStage : public PostProcessingStage {
 ...
 private:
  ...
  // Here we convert the dimensions to pixel locations in the lores image, as if
  // subsampled by hskip and vskip.
...
char const *MotionDetectStage::Name() const { return NAME; }
...
void MotionDetectStage::Configure() {
  ...
  // Turn fractions of the lores image into actual pixel numbers. Store them as
  // if in an image subsampled by hskip and vskip.
  ...
  region_threshold_ =
      std::clamp(region_threshold_, 0u, roi_width_ * roi_height_);

  if (config_.verbose)
    LOG(1, "Lores: " << info.width << "x" << info.height << " roi: (" << roi_x_
                     << "," << roi_y_ << ") " << roi_width_ << "x"
                     << roi_height_ << " threshold: " << region_threshold_);
...
bool MotionDetectStage::Process(CompletedRequestPtr &completed_request) {
  ...
  if (first_time_) {
    ...
    for (unsigned int y = 0; y < roi_height_; y++) {
      uint8_t *new_value_ptr =
          image + (roi_y_ + y) * lores_stride_ + roi_x_ * config_.hskip;
      uint8_t *old_value_ptr = &previous_frame_[0] + y * roi_width_;
      for (unsigned int x = 0; x < roi_width_;
           x++, new_value_ptr += config_.hskip)
  ...
  // Count the lores pixels where the difference between the new and previous
  // values exceeds the threshold. At the same time, update the previous image
  // buffer.
  for (unsigned int y = 0; y < roi_height_; y++) {
    uint8_t *new_value_ptr =
        image + (roi_y_ + y) * lores_stride_ + roi_x_ * config_.hskip;
    ...
    for (unsigned int x = 0; x < roi_width_;
      ...
      *(old_value_ptr++) = new_value;
      regions += std::abs(new_value - old_value) >
                 config_.difference_m * old_value + config_.difference_c;
  ...
  if (config_.verbose && motion_detected != motion_detected_)
    LOG(1, "Motion " << (motion_detected ? "detected" : "stopped")
                     << (config_.region_name.empty()
                             ? ""
                             : " in region " + config_.region_name));
