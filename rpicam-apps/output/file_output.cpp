...
FileOutput::~FileOutput() { closeFile(); }

void FileOutput::outputBuffer(void *mem, size_t size, int64_t timestamp_us,
                              uint32_t flags) {
  // We need to open a new file if we're in "segment" mode and our segment is
  // full (though we have to wait for the next I frame), or if we're in "split"
  // mode and recording is being restarted (this is necessarily an I-frame
  // already).
  if (fp_ == nullptr ||
...
void FileOutput::openFile(int64_t timestamp_us) {
  if (options_->output == "-")
    ...
    char filename[256];
    int n =
        snprintf(filename, sizeof(filename), options_->output.c_str(), count_);
    count_++;
    if (options_->wrap) count_ = count_ % options_->wrap;
    if (n < 0) throw std::runtime_error("failed to generate filename");
