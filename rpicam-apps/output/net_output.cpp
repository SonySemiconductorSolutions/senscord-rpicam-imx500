...
NetOutput::NetOutput(VideoOptions const *options) : Output(options) {
  ...
  if (sscanf(options->output.c_str(), "%3s://%n%d.%d.%d.%d%n:%d", protocol,
             &start, &a, &b, &c, &d, &end, &port) != 6)
  ...
  if (strcmp(protocol, "udp") == 0) {
    ...
    if (fd_ < 0) throw std::runtime_error("unable to open udp socket");
    ...
    if (options->listen) {
      ...
      if (bind(listen_fd, (struct sockaddr *)&server_saddr,
               sizeof(server_saddr)) < 0)
      ...
      if (fd_ < 0) throw std::runtime_error("accept socket failed");
      ...
      if (fd_ < 0) throw std::runtime_error("unable to open client socket");
    ...
    throw std::runtime_error("unrecognised network protocol " +
                             options->output);
}

NetOutput::~NetOutput() { close(fd_); }

// Maximum size that sendto will accept.
constexpr size_t MAX_UDP_SIZE = 65507;

void NetOutput::outputBuffer(void *mem, size_t size, int64_t /*timestamp_us*/,
                             uint32_t /*flags*/) {
