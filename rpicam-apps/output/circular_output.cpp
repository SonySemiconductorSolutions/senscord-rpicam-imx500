...
CircularOutput::CircularOutput(VideoOptions const *options)
  ...
  if (!fp_) throw std::runtime_error("could not open output file");
}

CircularOutput::~CircularOutput() {
  // We do have to skip to the first I frame before dumping stuff to disk. If
  // there are no I frames you will get nothing. Caveat emptor, methinks.
  ...
  while (!cb_.Empty()) {
    ...
    if (seen_keyframe) {
      cb_.Read([fp](void *src, int n) { fwrite(src, 1, n, fp); },
               header.length);
...
void CircularOutput::outputBuffer(void *mem, size_t size, int64_t timestamp_us,
                                  uint32_t flags) {
  // First make sure there's enough space.
  int pad = (ALIGN - size) & (ALIGN - 1);
  while (size + pad + sizeof(Header) > cb_.Available()) {
    if (cb_.Empty()) throw std::runtime_error("circular buffer too small");
    Header header;
    uint8_t *dst = (uint8_t *)&header;
    cb_.Read(
        [&dst](void *src, int n) {
  ...
  Header header = {static_cast<unsigned int>(size), !!(flags & FLAG_KEYFRAME),
                   timestamp_us};
...
void CircularOutput::timestampReady(int64_t timestamp) {
  // Don't want to save every timestamp as we go along, only outputs them at the
  // end
}
