...
Output::~Output() {
  if (fp_timestamps_) fclose(fp_timestamps_);
  if (!options_->metadata.empty())
    stop_metadata_output(buf_metadata_, options_->metadata_format);
}

void Output::Signal() { enable_ = !enable_; }

void Output::OutputReady(void *mem, size_t size, int64_t timestamp_us,
                         bool keyframe) {
  ...
  if (flags & FLAG_RESTART) time_offset_ = timestamp_us - last_timestamp_;
  ...
  if (!options_->metadata.empty()) {
    ...
    write_metadata(buf_metadata_, options_->metadata_format, metadata,
                   !metadata_started_);
...
void Output::timestampReady(int64_t timestamp) {
  fprintf(fp_timestamps_, "%" PRId64 ".%03" PRId64 "\n", timestamp / 1000,
          timestamp % 1000);
  if (options_->flush) fflush(fp_timestamps_);
}

void Output::outputBuffer(void *mem, size_t size, int64_t timestamp_us,
                          uint32_t flags) {
  // Supply this so that a vanilla Output gives you an object that outputs no
  // buffers.
}

Output *Output::Create(VideoOptions const *options) {
  if (options->codec == "libav" ||
      (options->codec == "h264" && options->GetPlatform() != Platform::VC4))
...
void Output::MetadataReady(libcamera::ControlList &metadata) {
  if (options_->metadata.empty()) return;
...
void write_metadata(std::streambuf *buf, std::string fmt,
                    libcamera::ControlList &metadata, bool first_write) {
  std::ostream out(buf);
  ...
  if (fmt == "txt") {
    ...
    if (!first_write) out << "," << std::endl;
    out << "{";
    bool first_done = false;
    for (auto const &[id, val] : metadata) {
      std::string arg_quote =
          (val.toString().find('/') != std::string::npos) ? "\"" : "";
      out << (first_done ? "," : "") << std::endl
          << "    \"" << id_map->at(id)->name() << "\": " << arg_quote
...
void stop_metadata_output(std::streambuf *buf, std::string fmt) {
  std::ostream out(buf);
  if (fmt == "json") out << std::endl << "]" << std::endl;
}
