...
struct TimeVal {
  ...
  void set(const std::string &s) {
    ...
    try {
      ...
      value =
          std::chrono::duration_cast<std::chrono::nanoseconds>(f * DEFAULT{1});
      ...
      for (const auto &m : match) {
        ...
        if (found != end_pos || found + m.first.length() != s.length())
          continue;
        value =
            std::chrono::duration_cast<std::chrono::nanoseconds>(f * m.second);
        break;
      }
...
struct Options {
  ...
  float saturation;
  float sharpness;
  std::optional<float> isp_framerate;
