...
void RPiCamApp::ConfigureViewfinder() {
  ...
  if (have_lores_stream) {
    ...
    configuration_->at(lores_stream_num).colorSpace =
        configuration_->at(0).colorSpace;
  }
...
void RPiCamApp::ConfigureVideo(unsigned int flags) {
  ...
  if (have_lores_stream) {
    ...
    configuration_->at(lores_index).bufferCount =
        configuration_->at(0).bufferCount;
    configuration_->at(lores_index).colorSpace =
        configuration_->at(0).colorSpace;
  }
  configuration_->orientation =
      libcamera::Orientation::Rotate0 * options_->transform;
...
void RPiCamApp::StartCamera() {
  ...
  if (!controls_.get(controls::FrameDurationLimits)) {
    ...
    else if (!options_->isp_framerate || options_->isp_framerate.value() > 0) {
      int64_t frame_time =
          1000000 / options_->isp_framerate.value_or(DEFAULT_FRAMERATE);  // in us
      controls_.set(
...
void RPiCamApp::previewThread() {
  while (true) {
    ...
    // if (item.stream->configuration().pixelFormat !=
    // libcamera::formats::YUV420) 	throw std::runtime_error("Preview windows
    // only support YUV420");
