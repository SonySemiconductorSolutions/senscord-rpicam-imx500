...
typedef std::function<void(void *, size_t, int64_t, bool)>
    EncodeOutputReadyCallback;
...
class RPiCamEncoder : public RPiCamApp {
 public:
  ...
  void StartEncoder() {
    createEncoder();
    encoder_->SetInputDoneCallback(std::bind(&RPiCamEncoder::encodeBufferDone,
                                             this, std::placeholders::_1));
    encoder_->SetOutputReadyCallback(encode_output_ready_callback_);

    // Set up the encode function to wait for synchronisation with another
    // camera system, when this has been requested in the options.
    VideoOptions const *options = GetOptions();
    libcamera::ControlList cl;
    if (options->sync == 0)
      cl.set(libcamera::controls::rpi::SyncMode,
             libcamera::controls::rpi::SyncModeOff);
    else if (options->sync == 1)
      cl.set(libcamera::controls::rpi::SyncMode,
             libcamera::controls::rpi::SyncModeServer);
    else if (options->sync == 2)
      cl.set(libcamera::controls::rpi::SyncMode,
             libcamera::controls::rpi::SyncModeClient);
    SetControls(cl);
  }
  ...
  bool EncodeBuffer(CompletedRequestPtr &completed_request, Stream *stream) {
    ...
    // If sync was enabled, and SyncReady is still "false" then we must skip
    // this frame. Tell our caller through the return value that we're not yet
    // encoding anything.
    if (GetOptions()->sync &&
        !completed_request->metadata.get(controls::rpi::SyncReady)
             .value_or(false))
      return false;
    ...
    void *mem            = span.data();
    if (!buffer || !mem) throw std::runtime_error("no buffer to encode");
    ...
    encoder_->EncodeBuffer(buffer->planes()[0].fd.get(), span.size(), mem, info,
                           timestamp_ns / 1000);
 ...
 private:
  void encodeBufferDone(void *mem) {
    ...
    // handle this by replacing the queue with a vector of <mem,
    // completed_request> pairs.)
    assert(mem == nullptr);
    {
      std::lock_guard<std::mutex> lock(encode_buffer_queue_mutex_);
