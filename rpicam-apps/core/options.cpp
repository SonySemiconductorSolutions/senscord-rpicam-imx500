...
Mode::Mode(std::string const &mode_string) : Mode() {
  if (!mode_string.empty()) {
    char p;
    int n = sscanf(mode_string.c_str(), "%u:%u:%u:%c", &width, &height,
                   &bit_depth, &p);
    if (n < 2)
...
std::string Mode::ToString() const {
  if (bit_depth == 0)
    ...
    std::stringstream ss;
    ss << width << ":" << height << ":" << bit_depth << ":"
       << (packed ? "P" : "U");
    if (framerate) ss << "(" << framerate << ")";
    return ss.str();
  }
}

void Mode::update(const libcamera::Size &size,
                  const std::optional<float> &fps) {
  if (!width) width = size.width;
  if (!height) height = size.height;
  if (!bit_depth) bit_depth = 12;
...
static bool set_imx708_subdev_hdr_ctrl(int en, const std::string &cam_id) {
  for (unsigned int i = 0; i < 16; i++) {
    const fs::path test_dir{"/sys/class/video4linux/v4l-subdev" +
...
Platform get_platform() {
  ...
  for (unsigned int device_num = 0; device_num < 256; device_num++) {
    ...
    if (!strncmp((char *)caps.driver, "uvcvideo", sizeof(caps.card))) continue;
...
Options::Options()
    : set_default_lens_position(false),
      af_on_capture(false),
      options_("Valid options are", 120, 80),
      app_(nullptr) {
  ...
  // clang-format off
	options_.add_options()
		...
		("isp_framerate", value<float>(&isp_framerate_)->default_value(-1.0),
			"Set the fixed framerate for ISP")
...
bool Options::Parse(int argc, char *argv[]) {
  ...
  if (framerate_ != -1.0) framerate = framerate_;
  ...
  if (nopreview && vm["info-text"].defaulted()) info_text = "";
  ...
  if (version) {
    ...
    std::cout << "rpicam-apps capabilites: " << RPiCamAppsCapabilities()
              << std::endl;
    std::cout << "libcamera build: " << libcamera::CameraManager::version()
              << std::endl;
    return false;
  }
  ...
  if (!log_env_set) libcamera::logSetLevel("*", "ERROR");
  ...
  if (camera < cameras.size()) {
    ...
    if (cam_id.find("imx708") != std::string::npos) {
      // HDR control. Set the sensor control before opening or listing any
      // cameras. Start by disabling HDR unconditionally. Reset the camera
      // manager if we have actually switched the value of the control
      bool changed = set_imx708_subdev_hdr_ctrl(0, cameras[camera]->id());

      if (hdr == "sensor" || hdr == "auto") {
        // Turn on sensor HDR.  Reset the camera manager if we have switched the
  ...
  if (list_cameras) {
    ...
    if (cameras.size() != 0) {
      ...
      for (auto const &cam : cameras) {
        ...
        if (!formats.pixelformats().size()) continue;

        unsigned int bits = 0;
        for (const auto &pix : formats.pixelformats()) {
          const auto &b = bayer_formats.find(pix);
          if (b != bayer_formats.end() && b->second > bits) bits = b->second;
        }
        if (bits) sensor_props << bits << "-bit ";

        auto cfa =
            cam->properties().get(properties::draft::ColorFilterArrangement);
        if (cfa && cfa_map.count(*cfa)) sensor_props << cfa_map.at(*cfa) << " ";
        ...
        for (const auto &pix : formats.pixelformats()) {
          ...
          for (const auto &size : formats.sizes(pix)) {
            ...
            auto crop_ctrl = cam->controls()
                                 .at(&controls::ScalerCrop)
                                 .max()
                                 .get<Rectangle>();
            ...
            std::cout << std::fixed << std::setprecision(2) << "[" << fps
                      << " fps - " << crop_ctrl.toString() << " crop" << "]";
            if (--num) {
              std::cout << std::endl;
              for (std::size_t s = 0; s < mode.length() + 11;
                   std::cout << " ", s++);
            }
          }
          std::cout << std::endl;
        ...
        if (verbose > 1) {
          ...
          std::cout << ss.str();
          for (std::size_t s = 0; s < ss.str().length() - 10;
               std::cout << "-", s++);
          std::cout << std::endl;
          ...
          for (auto const &c : ctrls) std::cout << "    " << c << std::endl;
        }
  ...
  if (verbose && !log_env_set) libcamera::logSetLevel("*", "INFO");

  // Set the verbosity
  RPiCamApp::verbosity = verbose;

  if (sscanf(preview.c_str(), "%u,%u,%u,%u", &preview_x, &preview_y,
             &preview_width, &preview_height) != 4)
    preview_x = preview_y = preview_width = preview_height =
        0;  // use default window

  transform = Transform::Identity;
  if (hflip_) transform = Transform::HFlip * transform;
  if (vflip_) transform = Transform::VFlip * transform;
  bool ok;
  Transform rot = transformFromRotation(rotation_, &ok);
  if (!ok) throw std::runtime_error("illegal rotation value");
  transform = rot * transform;
  if (!!(transform & Transform::Transpose))
    throw std::runtime_error("transforms requiring transpose not supported");

  if (sscanf(roi.c_str(), "%f,%f,%f,%f", &roi_x, &roi_y, &roi_width,
             &roi_height) != 4)
    roi_x = roi_y = roi_width = roi_height = 0;  // don't set digital zoom

  if (sscanf(afWindow.c_str(), "%f,%f,%f,%f", &afWindow_x, &afWindow_y,
             &afWindow_width, &afWindow_height) != 4)
    afWindow_x = afWindow_y = afWindow_width = afWindow_height =
        0;  // don't set auto focus windows
...
void Options::Print() const {
  ...
  if (nopreview)
    ...
    std::cerr << "    preview: " << preview_x << "," << preview_y << ","
  ...
  if (gain) std::cerr << "    gain: " << gain << std::endl;
  std::cerr << "    metering: " << metering << std::endl;
  std::cerr << "    exposure: " << exposure << std::endl;
  if (flicker_period)
    std::cerr << "    flicker period: " << flicker_period.get() << "us"
              << std::endl;
  ...
  std::cerr << "    framerate: " << framerate.value_or(DEFAULT_FRAMERATE)
            << std::endl;
  ...
  std::cerr << "    tuning-file: "
            << (tuning_file == "-" ? "(libcamera)" : tuning_file) << std::endl;
  ...
  if (afWindow_width == 0 || afWindow_height == 0)
    ...
    std::cerr << "    autofocus-window: " << afWindow_x << "," << afWindow_y
              << "," << afWindow_width << "," << afWindow_height << std::endl;
  ...
  std::cerr << "    viewfinder-mode: " << viewfinder_mode.ToString()
            << std::endl;
  if (buffer_count > 0)
    std::cerr << "    buffer-count: " << buffer_count << std::endl;
  if (viewfinder_buffer_count > 0)
    std::cerr << "    viewfinder-buffer-count: " << viewfinder_buffer_count
              << std::endl;
