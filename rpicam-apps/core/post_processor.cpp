...
const void *PostProcessingLib::GetSymbol(const std::string &symbol) {
  ...
  if (it == symbol_map_.end()) {
    ...
    if (!fn) {
      LOG_ERROR("Unable to find postprocessing symbol "
                << symbol << " with error: " << dlerror());
      return nullptr;
    }
...
PostProcessor::~PostProcessor() {
  // Must clear stages_ before dynamic_stages_ as the latter will unload the
  // necessary symbols.
  stages_.clear();
  ...
  for (auto const &key_and_value : root) {
    if (key_and_value.first == "rpicam-apps") {
      ...
      if (node.find("lores") != node.not_found()) {
        ...
        bool lores_par =
            node.get<bool>("lores.par", app_->GetOptions()->lores_par);
        std::string lores_format_str =
            node.get<std::string>("lores.format", "yuv420");
        ...
        LOG(1, "Postprocessing requested lores: " << lores_width << "x"
                                                  << lores_height << " "
                                                  << lores_format);
      }
    } else {
      PostProcessingStage *stage =
          createPostProcessingStage(key_and_value.first.c_str());
      if (stage) {
        LOG(1,
            "Reading post processing stage \"" << key_and_value.first << "\"");
...
PostProcessingStage *PostProcessor::createPostProcessingStage(
    char const *name) {
...
void PostProcessor::AdjustConfig(std::string const &use_case,
...
void PostProcessor::Process(CompletedRequestPtr &request) {
  ...
  requests_.push(
      std::move(request));  // caller has given us ownership of this reference

  std::promise<bool> promise;
  auto process_fn = [this](CompletedRequestPtr &request,
                           std::promise<bool> promise) {
  ...
  // Queue the futures to ensure we have correct ordering in the output thread.
  // The promise/future return value tells us when all the streams for this
  // request have been processed and output_ready_callback_ can be called.
  futures_.push(promise.get_future());
  std::thread{process_fn, std::ref(requests_.back()), std::move(promise)}
      .detach();
}
    ...
    bool drop_request = false;
    {
      ...
      cv_.wait(l, [this] {
        return (quit_ && futures_.empty()) ||
               (!futures_.empty() &&
                futures_.front().wait_for(0s) == std::future_status::ready);
      });

      // Only quit when the futures_ queue is empty.
      if (quit_ && futures_.empty()) break;

      drop_request = futures_.front().get();
      futures_.pop();
      request = std::move(
          requests_.front());  // reuse as it's being dropped from the queue
      requests_.pop();
    }
